...
### Third thousand reviewed variants

This batch adds 1,000 variants across 366 existing destinations, bringing the catalog to 1,013,136 mappings. It combines 409 corrections from codespell 2.4.1 with 591 from the pinned Norvig snapshot. There are 658 single-edit pairs and 342 two-edit pairs, including `authenatication` → `authentication`, `reopository` → `repository`, `depandencies` → `dependencies`, and `sycronization` → `synchronization`. All previous mappings and destination words are preserved.

Selection considers existing destinations occurring at least once per million words, including less common vocabulary and computing terms. It prioritizes codespell, then edit count, decreasing destination frequency, decreasing Norvig observation count where available, and alphabetical destination and typo. This batch admits at most six variants per destination and stops at 1,000. Two-edit pairs retain the minimum length of seven letters and matching first and last letters. Dictionary protection, normalized corpus exclusions, manual review, and the existing ambiguity rules still apply. Conflicting source corrections are excluded; for example, Norvig associates `consciencious` with several words, so codespell's proposed correction is held out. No new corpus-token exceptions are needed. The lower destination frequency floor does not change the frequency threshold for resolving ambiguity: rare destinations still need to qualify without that preference.
...
During Insert/Replace mode in enabled buffers, `vim.on_key` installs at most two native abbreviations and reads a bounded region around the cursor for dictionary lookup. Context checks run only for dictionary matches. Markdown delimiter tracking caches states for preceding lines and invalidates them from the first edited line; the initial check can scan from the buffer start. Most Markdown prose avoids parsing. Candidates inside fences trigger a synchronous parse with injections restricted to the candidate's line. The Markdown tree must confirm fence content, and the embedded language must identify a comment in a tree without parse errors; secondary injections inside strings cannot authorize correction. Indented Markdown and programming comments also use a synchronous Tree-sitter refresh; comment checks inspect the containing comment and adjacent preceding comment lines for code delimiters. Existing syntax groups provide the standalone comment fallback. Neovim handles expansion, preserving punctuation, undo/redo, macros, and Ctrl-V bypass. User abbreviations take precedence. Avoid scanning or installing the full catalog during editing.
...
`make check` and `make test` build the local cache first, so both work from a fresh checkout. Checks cover all mappings, capitalization, native abbreviation behavior, Markdown code and embedded comments, programming comments, parser/syntax fallbacks, live delimiter edits, boundaries, long lines, shared Lua/Python validation fixtures, audit consistency, automatic builds, cache reuse and invalidation, read-only installations, snapshots, symlinks, determinism, and recovery from invalid input or truncated cache data. Context tests use Neovim's bundled Markdown, Lua, and C parsers. Injection tests also exercise Python and Bash when those parsers are installed, reporting explicit skips otherwise. Benchmarks isolate plugin costs; compare results on the same Neovim version and machine. Building and normal editing require no Python.
